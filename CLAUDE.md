...
1. **extract_bacpac.py** — Installs sqlpackage via winget, imports `.bacpac` into local SQL Server, exports 14 tables to `csv_export/` as quoted CSV. Windows-only, requires SQL Server with Windows Auth.
...
- **Notebooks are generated, not hand-edited.** To change `wwi_load_from_csv.ipynb`, edit `build_csv_notebook.py` and re-run it.
